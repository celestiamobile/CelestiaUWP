...
namespace winrt::CelestiaComponent::implementation
{
    ...
    hstring GetBodyOverview(CelestiaBody const& body, CelestiaComponent::LocalizationProvider const& localizationProvider, CelestiaAppCore const& appCore)
    {
        ...
        {
            if (radius > oneMiInKm)
                radiusString = fmt::sprintf(std::wstring(localizationProvider(L"%s mi")), std::wstring(numberFormatter.Format(static_cast<int64_t>(radius / oneMiInKm))));
            else
                radiusString = fmt::sprintf(std::wstring(localizationProvider(L"%s ft")), std::wstring(numberFormatter.Format(static_cast<int64_t>(radius / oneFtInKm))));
        }
        else
        {
            if (radius > 1.0f)
                radiusString = fmt::sprintf(std::wstring(localizationProvider(L"%s km")), std::wstring(numberFormatter.Format(static_cast<int64_t>(radius))));
            else
                radiusString = fmt::sprintf(std::wstring(localizationProvider(L"%s m")), std::wstring(numberFormatter.Format(static_cast<int64_t>(radius * 1000.0f))));
        }
        if (body.IsEllipsoid())
        ...
        if (rotation.IsPeriodic() && body.Type() != CelestiaBodyType::spacecraft)
        {
            ...
            if (rotPeriod < 2.0)
            {
                ...
                unitTemplate = localizationProvider(L"%s hours");
            }
            else
            {
                unitTemplate = localizationProvider(L"%s days");
            }
            rotPeriod = std::round(rotPeriod * 100.0) / 100.0;
            dayLength = std::round(dayLength * 100.0) / 100.0;
            lines.push_back(fmt::sprintf(std::wstring(localizationProvider(L"Sidereal rotation period: %s")), fmt::sprintf(std::wstring(unitTemplate), std::wstring(numberFormatter.Format(rotPeriod)))));

            if (dayLength != 0.0)
            {
                lines.push_back(fmt::sprintf(std::wstring(localizationProvider(L"Length of day: %s")), fmt::sprintf(std::wstring(unitTemplate), std::wstring(numberFormatter.Format(dayLength)))));
            }
        }
        if (body.HasRings())
        {
        ...
        if (timeline.PhaseCount() > 0)
        {
            ...
            if (!std::isinf(startJulianDay))
            {
                if (startJulianDay < CelestiaHelper::MinRepresentableJulianDay() || startJulianDay > CelestiaHelper::MaxRepresentableJulianDay())
                {
                    ...
                    lines.push_back(fmt::sprintf(std::wstring(localizationProvider(L"Start Julian day: %s")), std::wstring(numberFormatter.Format(startJulianDay))));
                }
                else
                {
            ...
            if (!std::isinf(endJulianDay))
            {
                if (endJulianDay < CelestiaHelper::MinRepresentableJulianDay() || endJulianDay > CelestiaHelper::MaxRepresentableJulianDay())
                {
                    ...
                    lines.push_back(fmt::sprintf(std::wstring(localizationProvider(L"End Julian day: %s")), std::wstring(numberFormatter.Format(endJulianDay))));
                }
                else
                {
    ...
    hstring GetStarOverview(CelestiaStar const& star, CelestiaComponent::LocalizationProvider const& localizationProvider, CelestiaAppCore const& appCore)
    {
        ...
        lines.push_back(fmt::sprintf(std::wstring(localizationProvider(L"RA: %sh %sm %ss")), std::wstring(numberFormatter.Format(hms.Hours())), std::wstring(numberFormatter.Format(hms.Minutes())), std::wstring(numberFormatter.Format(seconds))));

        CelestiaDMS dms{ sph.Y() };
        seconds = std::round(dms.Seconds() * 100.0) / 100.0;
        lines.push_back(fmt::sprintf(std::wstring(localizationProvider(L"DEC: %s\u00b0 %s\u2032 %s\u2033")), std::wstring(numberFormatter.Format(dms.Hours())), std::wstring(numberFormatter.Format(dms.Minutes())), std::wstring(numberFormatter.Format(seconds))));
    ...
    hstring GetDSOOveriew(CelestiaDSO const& dso, CelestiaComponent::LocalizationProvider const& localizationProvider, CelestiaAppCore const&)
    {
        ...
        seconds = std::round(dms.Seconds() * 100.0) / 100.0;
        lines.push_back(fmt::sprintf(std::wstring(localizationProvider(L"L: %s\u00b0 %s\u2032 %s\u2033")), std::wstring(numberFormatter.Format(dms.Hours())), std::wstring(numberFormatter.Format(dms.Minutes())), std::wstring(numberFormatter.Format(seconds))));

        dms = CelestiaDMS(sph.Y());
        seconds = std::round(dms.Seconds() * 100.0) / 100.0;
        lines.push_back(fmt::sprintf(std::wstring(localizationProvider(L"B: %s\u00b0 %s\u2032 %s\u2033")), std::wstring(numberFormatter.Format(dms.Hours())), std::wstring(numberFormatter.Format(dms.Minutes())), std::wstring(numberFormatter.Format(seconds))));
