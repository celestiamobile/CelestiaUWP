...
namespace winrt::CelestiaWinUI::implementation
{
    ...
    bool MainWindow::StartEngine(hstring const& resourcePath, hstring const& configPath, hstring const& locale, JsonObject const& defaultSettings)
    {
        ...
        if (!loadSuccess)
        {
            if (resourcePath != defaultResourcePath || configPath != defaultConfigFilePath)
            {
                ...
                CelestiaAppCore::SetLocaleDirectory(defaultResourcePath + L"\\locale", locale);
        ...
        DispatcherQueue().TryEnqueue(Microsoft::UI::Dispatching::DispatcherQueuePriority::Normal, [this, resourcePath]()
            {
                ...
                if (extraAddonFolder != nullptr)
                    resourceManager = ResourceManager(extraAddonFolder);
    ...
    void MainWindow::PopulateMenuBar(hstring const& resourcePath)
    {
        ...
        auto scripts = CelestiaAppCore::ReadScripts(resourcePath + L"\\scripts", true);
        ...
        if (!extraScriptFolder.empty())
        {
            auto extraScripts = CelestiaAppCore::ReadScripts(extraScriptFolder, true);
    ...
    IAsyncAction MainWindow::ShowGotoObject()
    {
        ...
        if (co_await ContentDialogHelper::ShowContentDialogAsync(Content(), dialog) == ContentDialogResult::Primary)
        {
            ...
            renderer.EnqueueTask([this, objectName, latitude, longitude, distance, unit]
                {
                    auto selection = appCore.Simulation().Find(objectName);
    ...
    fire_and_forget MainWindow::ShowObserverMode()
    {
        ...
        renderer.EnqueueTask([this, coordinateSystem, referenceObjectName, targetObjectName]()
            {
                auto referenceObject = referenceObjectName.empty() ? CelestiaSelection() : appCore.Simulation().Find(referenceObjectName);
                auto targetObject = targetObjectName.empty() ? CelestiaSelection() : appCore.Simulation().Find(targetObjectName);
    ...
    void MainWindow::ShowBookmarkOrganizer()
    {
        ...
        BookmarkOrganizerUserControl userControl{ appCore, renderer };
        Window window;
    ...
    void MainWindow::ShowSettings()
    {
        ...
        SettingsUserControl userControl{ appCore, AppSettings(), ApplicationData::Current().LocalSettings(), languages };
        Window window;
